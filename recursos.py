...
class HiloDescargador(threading.Thread):
    ...
    def __init__(self, nombre_hilo, url, ruta, ydl_opts,list_progreso_descarga,item, contendor_descarga,daemon=None, key_des=None):
        ...
        self.list_progreso_descarga=list_progreso_descarga  # agrego un diccionario de progreos a una lista global para ir viendo los prograsos independiente
    ...
    def progreso_descarga(self, d):
        ...
        for progreso in self.list_progreso_descarga:
            if self.name in progreso:
                ...
                if d['status']=='downloading':
                    ...
                    progreso[self.name][0].ids.lbl_porcentaje.text=str(porcentaje)+"% velocidad "+str(d['_speed_str'])+" formato de salida: "+progreso["formato"]
        ...
        if d['status'] == 'finished':
            for descarga in self.list_progreso_descarga:
                if self.name in descarga:
                    descarga.pop(self.name)
            print('descarga finalizada, convertido, y eliminado de la lista')


    def run(self):
        url_descarga = self.url
        self.ydl_opts['logger']=MyLogger()#añadiendole el logger
        self.ydl_opts['progress_hooks']=[self.progreso_descarga]#llamando a la funcion encargada del progreso de cada MDProgressBar
